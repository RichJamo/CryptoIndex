const DAPP_ADDRESS = "0xf89D4A1D6C498b207FE6F1B64a9d0B712bd0d8Bf"; //insert the address I deployed to
...
async function startApp(provider) {
  ...
  depositButton.addEventListener('click', async () => {
    ...
    var token_addresses = [BTCB_ADDRESS, WETH_ADDRESS, BNB_ADDRESS, ADA_ADDRESS, XRP_ADDRESS, SOL_ADDRESS, DOT_ADDRESS, DOGE_ADDRESS, UNI_ADDRESS, LTC_ADDRESS];
...
async function displayBalances() {
  ...
  getBTCBResult.innerHTML = parseFloat(ethers.utils.formatUnits(await getBalance(BTCB_ADDRESS, DAPP_ADDRESS), 18)).toFixed(6) || 'Not able to get accounts';
  ...
  getSOLResult.innerHTML = parseFloat(ethers.utils.formatUnits(await getBalance(SOL_ADDRESS, DAPP_ADDRESS), 18)).toFixed(6) || 'Not able to get accounts';
...
async function displayUSDBalances() {
  ...
  BTCBInUsd.innerHTML = btcb_usd.toFixed(2) || 'Not able to get accounts'; //8 decimals for oracle input, 18 for WMATIC
  ...
  var sol_usd = array_coins[5].usd_balance;
  SOLInUsd.innerHTML = sol_usd.toFixed(2) || 'Not able to get accounts';
  ...
  var total_in_usd = btcb_usd + weth_usd + bnb_usd + ada_usd + xrp_usd + sol_usd + dot_usd + doge_usd + uni_usd + ltc_usd;
...
async function getTokenInfo(accountOrContract) {
  ...
  var tokenList = [BTCB_ADDRESS, WETH_ADDRESS, BNB_ADDRESS, ADA_ADDRESS, XRP_ADDRESS, SOL_ADDRESS, DOT_ADDRESS, DOGE_ADDRESS, UNI_ADDRESS, LTC_ADDRESS];
  var array_coins = [];
  var i=0;
  ...
  // var SOL = new Coin("SOL", SOL_ADDRESS, SOL_USD_ORACLE);
  ...
  // var array_coins = [BTCB, WETH, BNB, ADA, XRP, SOL, DOT, DOGE, UNI, LTC];
  var total_in_usd = 0;
